...
// void NativeEval_f(ZZ &y, int d, int num_data, int loop, int beg_ind, int *ind_var, vec_ZZ X, vec_ZZ F,int &exec_count) {
//     if (loop == d) {
//         ZZ temp;
//         temp=F[exec_count%10];
//         for (int i = 0; i < d; i++) {
...
//             ind_var[loop - 1] = i;
//             NativeEval_f(y, d, num_data, loop, i, ind_var, X, F, exec_count);
//         }
//     }

// }

// void NativeEval(ZZ &y, int d, int num_data, vec_ZZ X, vec_ZZ F) {
//     ZZ temp;
//     int count=0;
//     for (int i = 1; i < d + 1; i++) {
//         int *ind_var = (int *) malloc(sizeof(int) * i);
//         int exec_count = 0; // Initialize execution count
//         NativeEval_f(temp, i, num_data, 0, 0, ind_var, X, F, count);
//         y = y + temp;
//         temp = 0;
//         free(ind_var); // Free allocated memory
//     }
//     cout<<"total items:"<<count<<endl;
// }


void NativeEval(ZZ &y, int d, int num_data, vec_ZZ X, vec_ZZ F, vector<vector<int>> &F_coefficient, int item) {
    ZZ temp;
    int count=0;
