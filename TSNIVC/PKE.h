...
struct PKE_Para {
    ...
    ZZ msg,p,q,coeff,twice_p,twice_q,half_p,conversion_modulus;
    ZZ_pX xN;
    int hsk = 64;
    int d;
    int num_data;
...
void PKE_DDec(vec_ZZ_pX &db, const PKE_Para pkePara, ZZ_pXModulus modulus,vec_ZZ_pX pkeSk, vec_ZZ_pX C) {
    ...
    conv(db[0],temp);
    MulMod(temp1, pkeSk[2], C[0], modulus);
    MulMod(temp2, pkeSk[3], C[1], modulus);
    temp1 = temp1 + temp2;
    conv(temp,temp1);
    for (int i = 0; i < pkePara.N; i++) {
        GetCoeff(coeff, temp, i);
...
void PKE_DDec2(vec_ZZ_pX &db, const PKE_Para pkePara, ZZ_pXModulus modulus,vec_ZZ_pX pkeSk, vec_ZZ_pX C) {
    ZZ coeff;
    ZZX temp;
    ZZ_pX temp1, temp2;
    MulMod(temp1, pkeSk[0], C[0], modulus);
