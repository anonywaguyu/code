#include "TSNIVC.h"
#include "GenData.h"
#include <algorithm>
#include <cstdlib>
#include <NTL/ZZ.h>
#include <NTL/ZZX.h>
#include <NTL/ZZ_pX.h>

using namespace std;

void Set_pX(ZZ_pX &a, int N, int q_bit) {
    ZZ_p coeff;
    for (int i = 0; i < N; i++) {
        conv(coeff, RandomBits_ZZ(q_bit)<<213);
        SetCoeff(a, i, coeff);

    }
}

void Eval_Poly(int deg, int num_data) {
    cout << "*************************** Setup -***************************" << "\n";
    //para
    PKE_Para pkePara;
    vec_ZZ_pX pkePk, pkeSk, hssEk_1, hssEk_2;
    pkePk.SetLength(2);
    pkeSk.SetLength(2);
    hssEk_1.SetLength(4);
    hssEk_2.SetLength(4);
    pkePara.msg_bit = 32;
    pkePara.d = deg;
    pkePara.num_data = num_data;
    Setup(hssEk_1, hssEk_2, pkePara, pkePk, pkeSk,1);
    ZZ_pXModulus modulus(pkePara.xN);
    //C^1
    ZZ constant1=conv<ZZ>("1");
    vec_ZZ_pX C1;
    C1.SetLength(4);
    PKE_OKDM(C1, pkePara, modulus, pkePk, constant1);
    ZZ r;
    GenPrime(r, 32);
    
    int k=pkePara.d+1;
    //data
    Data data;
    cout << "*************************** Keygen ***************************" << "\n";
    GenDataF(data, pkePara, pkePk, r);
    GenDataX(data, pkePara, pkePk, r);
    vec_ZZ y, y1, y2;
    y.SetLength(k);
    y1.SetLength(k);
    y2.SetLength(k);
    
    
    //C^a
    vec_ZZ_pX Ca;
    Ca.SetLength(2);
    Random_ZZ_pX(Ca[1], pkePara.N, 128);

    cout << "************************** Probgen ***************************" << "\n";
    ZZ tba1, tba2, coeff, y_native, c_1;
    vec_ZZ pi, tbpi1, tbpi2, ay, pi_,gamma,alpha,v,w;
    pi.SetLength(k);
    tbpi1.SetLength(k);
    tbpi2.SetLength(k);
    ay.SetLength(k);
    pi_.SetLength(k);
    gamma.SetLength(pkePara.num_data);
    alpha.SetLength(k);
    v.SetLength(k);
    for(int j=0;j<k;j++)v[j]=conv<ZZ>("1");
    w.SetLength(k);
    for(int j=0;j<k;j++)w[j]=conv<ZZ>("1");
    ZZ y_final=conv<ZZ>("0");
    ZZX ytemp;
    double time = GetTime();
    for(int j=0;j<pkePara.num_data;j++)RandomBits(gamma[j], pkePara.msg_bit);
    for(int j=0;j<k;j++)RandomBits(alpha[j], pkePara.msg_bit);
    for (int i = 0; i < pkePara.num_data; i++) {
        for(int j=0;j<k;j++)
        {
            data.XX[j][i]=(data.X[i]+alpha[j]*gamma[i]);
        }
    }
    for(int i=0;i<k;i++)
    {
        for(int j=0;j<k;j++)
        {
            if(j==i);
            else v[i]*=(alpha[i]-alpha[j]);
        }
    }
    for(int i=0;i<k;i++)
    {
        for(int j=0;j<k;j++)
        {
            if(j==i);
            else w[i]*=alpha[j];
        }
    }
    time = GetTime() - time;
    cout << "running time Probgen: " << time * 1000 << "ms\n";

    cout << "******************** Server 1 Evaluating ********************" << "\n";
    time = GetTime();
    Compute(y1, tba1, tbpi1, 1, pkePara, modulus, hssEk_1, data.XX, data.C_F, data.PRF, C1, Ca, r, v);
    time = GetTime() - time;
    cout << "running time Server 1: " << time * 1000 << "ms\n";

    cout << "******************** Server 2 Evaluating ********************" << "\n";
    time = GetTime();
    Compute(y2, tba2, tbpi2, 2, pkePara, modulus, hssEk_2, data.XX, data.C_F, data.PRF, C1, Ca, r, v);
    time = GetTime() - time; 
    cout << "running time Server 2: " << time * 1000 << "ms\n";
    
    ZZ a=RandomBits_ZZ(128);
    cout << "********************** Client Verifying and reconstrcuting **********************" << "\n";
    time = GetTime();
    c_1=(tba1+tba2+(a<<(pkePara.lift_bit)));
    for(int j=0;j<k;j++)
    {
        y[j] = (y1[j] + y2[j])%pkePara.p;
        pi[j] = (((y[j]*c_1+tbpi1[j]+tbpi2[j])*pkePara.twice_p+pkePara.q)>>(1+pkePara.q_bit))%pkePara.p;
        ay[j]=a*y[j]%pkePara.p;
        pi_[j] = ((((y[j]*c_1+tbpi1[j])*pkePara.twice_p+pkePara.q)>>(1+pkePara.q_bit))+
            ((tbpi2[j]*pkePara.twice_p+pkePara.q)>>(1+pkePara.q_bit)))%pkePara.p;

        if (pi[j]==ay[j] && pi[j]==pi_[j]) {
        } else {
            printf("******************** ERROR ********************\n");
            cout << "The value of y   is  :" << y << endl;
            printf("\n\n");
            cout << "THE value of pi  is  :" << pi <<endl;
        }
    }
    for(int j=0;j<k;j++)
    {
        y_final+=(w[j]*y[j]);
    }
    y_final%=r;
    time = GetTime() - time;
    cout << "running time Client: " << time * 1000 << "ms\n";


    cout << "******************** native computing ********************" << "\n";
    time = GetTime();
    NativeEval(y_native, pkePara.d, pkePara.num_data, data.X, data.F);
    time = GetTime()-time;
    cout << "native computing time: " << time * 1000 << "ms\n";
    cout << "The value of native y      :" << y_native%r << endl;
    cout << "The value of native y_final:" << y_final << endl;
}

void Run_Setup(int msg_bit, int num_data) {
    PKE_Para pkePara;
    vec_ZZ_pX pkePk, pkeSk, hssEk_1, hssEk_2;
    pkePk.SetLength(2);
    pkeSk.SetLength(2);
    hssEk_1.SetLength(4);
    hssEk_2.SetLength(4);
    pkePara.msg_bit = msg_bit;
    pkePara.num_data = 2;
    Setup(hssEk_1, hssEk_2, pkePara, pkePk, pkeSk,0);
}

void Time_Setup(int msg_bit, int cyctimes) {
    auto *Time = new double[cyctimes];
    double time,mean,stdev=1;
    while(stdev*100>10)
    {
        for (int i = 0; i < cyctimes; i++) {
        time = GetTime();
        Run_Setup(msg_bit, 2);
        Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    cout << "Setup algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";
}


void Time_Keygen(int msg_bit,  int cyctimes) {
    //para
    PKE_Para pkePara;
    vec_ZZ_pX pkePk, pkeSk, hssEk_1, hssEk_2;
    pkePk.SetLength(2);
    pkeSk.SetLength(2);
    hssEk_1.SetLength(4);
    hssEk_2.SetLength(4);
    pkePara.msg_bit = msg_bit;
    pkePara.num_data = 2;
    Setup(hssEk_1, hssEk_2, pkePara, pkePk, pkeSk, 0);
    ZZ_pXModulus modulus(pkePara.xN);

    vec_ZZ_pX C;
    ZZ x;
    C.SetLength(2);
    auto *Time = new double[cyctimes];
    double time,mean,stdev=1;
    while(stdev*100>10)
    {
        for (int i = 0; i < cyctimes; i++) {
        x = RandomBits_ZZ(pkePara.msg_bit);
        time = GetTime();
        Keygen(C, pkePara, modulus, pkePk, x);
        Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    cout << "Keygen algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";
}

void Time_Probgen(int msg_bit,  int cyctimes) {
    //para
    PKE_Para pkePara;
    vec_ZZ_pX pkePk, pkeSk, hssEk_1, hssEk_2;
    pkePk.SetLength(2);
    pkeSk.SetLength(2);
    hssEk_1.SetLength(4);
    hssEk_2.SetLength(4);
    pkePara.msg_bit = msg_bit;
    pkePara.num_data = 4;
    Setup(hssEk_1, hssEk_2, pkePara, pkePk, pkeSk, 0);
    ZZ_pXModulus modulus(pkePara.xN);

    //C^1
    ZZ constant1=conv<ZZ>("1");
    vec_ZZ_pX C1;
    C1.SetLength(4);
    PKE_OKDM(C1, pkePara, modulus, pkePk, constant1);
    ZZ r;
    GenPrime(r, 32);
    Data data;
    GenDataX(data, pkePara, pkePk, r);
    //pkePara.d is set to 4;
    ZZ tba1, tba2, coeff, y_native, c_1;
    vec_ZZ pi, tbpi1, tbpi2, ay, pi_,gamma,alpha,v,w;
    pi.SetLength(5);
    tbpi1.SetLength(5);
    tbpi2.SetLength(5);
    ay.SetLength(5);
    pi_.SetLength(5);
    gamma.SetLength(pkePara.num_data);
    alpha.SetLength(5);
    v.SetLength(5);
    for(int j=0;j<5;j++)v[j]=conv<ZZ>("1");
    w.SetLength(5);
    for(int j=0;j<5;j++)w[j]=conv<ZZ>("1");
    ZZ y_final=conv<ZZ>("0");
    ZZX ytemp;

    auto *Time = new double[cyctimes];
    double time,mean,stdev=1;
    while(stdev*100>10)
    {
        for (int c = 0; c < cyctimes; c++) 
        {
            time = GetTime();
            for(int j=0;j<pkePara.num_data;j++)RandomBits(gamma[j], 32);
            for(int j=0;j<5;j++)RandomBits(alpha[j], 32);
            for (int i = 0; i < pkePara.num_data; i++) {
                for(int j=0;j<5;j++)
                {
                    data.XX[j][i]=(data.X[i]+alpha[j]*gamma[i]);
                }
            }
            for(int i=0;i<5;i++)
            {
                for(int j=0;j<5;j++)
                {
                    if(j==i);
                    else v[i]*=(alpha[i]-alpha[j]);
                }
            }
            for(int i=0;i<5;i++)
            {
                for(int j=0;j<5;j++)
                {
                    if(j==i);
                    else w[i]*=alpha[j];
                }
            }
            Time[c] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    cout << "Probgen algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";
}

void Time_Verify(int msg_bit, int cyctimes)
{
    //para
    PKE_Para pkePara;
    vec_ZZ_pX pkePk, pkeSk, hssEk_1, hssEk_2,tb1,tb2;
    pkePk.SetLength(2);
    pkeSk.SetLength(2);
    hssEk_1.SetLength(4);
    hssEk_2.SetLength(4);
    tb1.SetLength(2);
    tb2.SetLength(2);
    pkePara.num_data = 2;
    Setup(hssEk_1, hssEk_2, pkePara, pkePk, pkeSk, 0);
    ZZ_pXModulus modulus(pkePara.xN);
    ZZX temp;
    ZZ_pX temp1;
    ZZ_pX e1;
    ZZ x,y,y1,y2,c_1,ay,pi,pi_;
    ZZ tba1,tba2,tbpi1,tbpi2,a;
    vec_ZZ_pX Ca,C;
    Ca.SetLength(2);
    C.SetLength(4);
    Random_ZZ_pX(Ca[1], pkePara.N, pkePara.q_bit);
    auto *Time = new double[cyctimes];
    double time,mean,stdev=1;
    while(stdev*100>30)
    {
        for (int i = 0; i < cyctimes; i++) {
            x = RandomBits_ZZ(32);
            PKE_OKDM(C, pkePara, modulus, pkePk, x);
            Load(tb1, pkePara, modulus, hssEk_1, C);
            conv(temp,tb1[0]);
            GetCoeff(y1, temp, 0);
            
            GaussRand(e1, pkePara.N);
            MulMod(temp1, hssEk_1[1], Ca[1], modulus);
            temp1=e1-temp1;
            conv(temp,temp1);
            GetCoeff(tba1, temp, 0);

            GaussRand(e1, pkePara.N);
            MulMod(temp1, tb1[1], Ca[1], modulus);
            temp1=e1+temp1;
            conv(temp,temp1);
            GetCoeff(tbpi1, temp, 0);

            Load(tb2, pkePara, modulus, hssEk_2, C);
            conv(temp,tb2[0]);
            GetCoeff(y2, temp, 0);

            GaussRand(e1, pkePara.N);
            MulMod(temp1, hssEk_2[1], Ca[1], modulus);
            temp1=e1-temp1;
            conv(temp,temp1);
            GetCoeff(tba2, temp, 0);

            GaussRand(e1, pkePara.N);
            MulMod(temp1, tb2[1], Ca[1], modulus);
            temp1=e1+temp1;
            conv(temp,temp1);
            GetCoeff(tbpi2, temp, 0);

            time = GetTime();
            a=RandomBits_ZZ(128);
            c_1=(tba1+tba2+(a<<(pkePara.lift_bit)));
            for(int j=0;j<5;j++){
                y = (y1 + y2)%pkePara.p;
                pi = (((y*c_1+tbpi1+tbpi2)*pkePara.twice_p+pkePara.q)>>(1+pkePara.q_bit))%pkePara.p;
                ay=a*y%pkePara.p;
                pi_ = ((((y*c_1+tbpi1)*pkePara.twice_p+pkePara.q)>>(1+pkePara.q_bit))+
                    ((tbpi2*pkePara.twice_p+pkePara.q)>>(1+pkePara.q_bit)))%pkePara.p;
                if (pi==ay && pi==pi_) {
                } else {
                    printf("******************** ERROR ********************\n");
                    cout << "The value of y   is  :" << y << endl;
                    printf("\n\n");
                    cout << "THE value of pi  is  :" << pi <<endl;
                }   
            }
            Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    
    cout << "Verify algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";
}

void Time_Recon(int msg_bit, int cyctimes)
{
    int k=5;
    vec_ZZ w,y;
    w.SetLength(k);
    y.SetLength(k);
    ZZ y_final=conv<ZZ>("0");
    ZZ r;
    GenPrime(r, 32);
    auto *Time = new double[cyctimes];
    double time,mean,stdev=1;
    while(stdev*100>30)
    {
        for (int i = 0; i < cyctimes; i++) {
            for(int j=0;j<k;j++)
            {
                w[j]=RandomBits_ZZ(32);
                w[j]%=r;
                y[j]=RandomBits_ZZ(32);
                y[j]%=r;
            } 
            time = GetTime();
            for(int j=0;j<k;j++)
            {
                y_final+=(w[j]*y[j]);
            }
            y_final%=r;
            Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    
    cout << "Recon algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";
}

void Time_Eval_Subalgo(int msg_bit,  int cyctimes) {
    //para
    PKE_Para pkePara;
    vec_ZZ_pX pkePk, pkeSk, hssEk_1, hssEk_2,tb1;
    pkePk.SetLength(2);
    pkeSk.SetLength(2);
    hssEk_1.SetLength(4);
    hssEk_2.SetLength(4);
    pkePara.msg_bit = msg_bit;
    pkePara.num_data = 2;
    Setup(hssEk_1, hssEk_2, pkePara, pkePk, pkeSk, 0);
    ZZ_pXModulus modulus(pkePara.xN);
    ZZ r;
    GenPrime(r, 32);
    //data
    Data data;
    GenData(data, pkePara, pkePk, r);
    //load
    vec_ZZ_pX db1,db2;
    db1.SetLength(2);
    db2.SetLength(2);
    auto *Time = new double[cyctimes];
    double time,mean,stdev=1;
    while(stdev*100>10)
    {
        for (int i = 0; i < cyctimes; i++) {
        int index=rand()%10;
        time = GetTime();
        Load(db1,pkePara,modulus,hssEk_1,data.C_X[rand()%pkePara.num_data]);
        db1[0]=db1[0]+data.PRF[index][0];
        db1[1]=db1[1]+data.PRF[index][1];
        Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    cout << "Load algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";


    //add1
    stdev=1;
    while(stdev*100>10)
    {
        for (int i = 0; i < cyctimes; i++) {
        Random_ZZ_pX(db1[0],pkePara.N,pkePara.q_bit);
        Random_ZZ_pX(db1[1],pkePara.N,pkePara.q_bit);
        Random_ZZ_pX(db2[0],pkePara.N,pkePara.q_bit);
        Random_ZZ_pX(db2[1],pkePara.N,pkePara.q_bit);
        int index=rand()%10;
        time = GetTime();
        db1[0]=db1[0]+db2[0]+data.PRF[index][0];
        db1[1]=db1[1]+db2[1]+data.PRF[index][1];
        Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    cout << "Add1 algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";

    //cmult
    ZZ constant= RandomBits_ZZ(msg_bit);
    stdev=1;
    while(100*stdev>10)
    {
        for (int i = 0; i < cyctimes; i++) {
        Load(db1, pkePara, modulus, hssEk_1, data.C_X[rand() % pkePara.num_data]);
        int index = rand() % 10;
        time = GetTime();
        ZZ_pX_ScaleMul_ZZ(db1[0],db1[0],constant);
        ZZ_pX_ScaleMul_ZZ(db1[1],db1[1],constant);
        db1[0] = db1[0] + data.PRF[index][0];
        db1[1] = db1[1] + data.PRF[index][1];
        Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    
    cout << "cMult algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";

    //out
    ZZX temp;
    ZZ_pX temp1;
    ZZ_pX e1;
    ZZ x,y,y1,tba1,tbpi1;
    vec_ZZ_pX Ca, C;
    Ca.SetLength(2);
    C.SetLength(4);
    tb1.SetLength(2);
    Random_ZZ_pX(Ca[1], pkePara.N, pkePara.q_bit);
    stdev=1;
    while(stdev*100>10)
    {
        for (int i = 0; i < cyctimes; i++) {
            x = RandomBits_ZZ(pkePara.msg_bit);
            PKE_OKDM(C, pkePara, modulus, pkePk, x);
            time = GetTime();
            Output(tb1, pkePara, modulus, hssEk_1, C);
            conv(temp,tb1[0]);
            GetCoeff(y1, temp, 0);

            GaussRand(e1, pkePara.N);
            MulMod(temp1, tb1[1], Ca[1], modulus);
            temp1=e1+temp1;
            conv(temp,temp1);
            GetCoeff(tbpi1, temp, 0);
            Time[i] = GetTime() - time;
        }
        DataProcess(mean,stdev,Time,cyctimes);
    }
    cout << "Output algo time: " << mean * 1000 << " ms  RSD: "<<stdev*100<<"%\n";

}

