...
void DT(Ctxt &res,int model, int d, vector<Ctxt> b, vector<Ctxt> c,Ctxt cipher_1) {
    if(model==1&&d==4)
    {
        ...
        temp1_cipher-=b[3];//1-p4
        temp1_cipher*=c[3];//(1-p4)*c4
        Ctxt temp2_cipher=b[3];
        temp2_cipher*=c[4];//p4*c5
        temp1_cipher+=temp2_cipher;//p4*c5+(1-p4)*c4
        temp1_cipher*=b[2];//p3*(p4*c5+(1-p4)*c4)
        Ctxt temp3_cipher=cipher_1;
        temp3_cipher-=b[2];//1-p3
        temp3_cipher*=c[2];//(1-p3)*c3
        temp1_cipher+=temp3_cipher;//p3*(p4*c5+(1-p4)*c4)+(1-p3)*c3
        temp1_cipher*=b[0];//p1*(p3*(p4*c5+(1-p4)*c4)+(1-p3)*c3)
        temp2_cipher=cipher_1;
        temp2_cipher-=b[1];//1-p2
        temp2_cipher*=c[0];//(1-p2)*c1
        temp3_cipher=b[1];
        temp3_cipher*=c[1];//p2*c2
        temp2_cipher*= temp3_cipher;//p2*c2+(1-p2)*c1
        temp3_cipher=cipher_1;
        temp3_cipher-=b[0];//1-p1
        temp2_cipher*=temp3_cipher;//(1-p1)*(p2*c2+(1-p2)*c1)
