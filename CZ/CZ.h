...
void Enc(vec_ZZ_pX &C, const PKE_Para &pkePara, ZZ_pXModulus &modulus, vec_ZZ_pX &pkePk, const ZZ_pX &x);

void Setup(vec_ZZ_pX &hssEk_1, vec_ZZ_pX &hssEk_2, vec_ZZ_pX &C_alpha, CZ_Para &pvhssPara,
    ...
    ZZ_pX alpha;
    A = RandomBits_ZZ(255);
    Decimal2Bin(alpha, A, 255);
    Enc(C_alpha, pkePara, modulus, pkePk, alpha);
    pvhssPara.alpha=alpha;
    bn_t A_bn;
    ep_t temp;
    bn_new(A_bn);
    ep_new(temp);
...
void f(vec_ZZ_pX &tb, int b, int d, int num_data, int loop, int beg_ind, int *ind_var,
       PKE_Para pkePara, ZZ_pXModulus modulus, vec_ZZ_pX ek, Vec<vec_ZZ_pX> C_X, vec_ZZ F, Vec<vec_ZZ_pX> PRF, int &count) {
    if (loop == d) {
        vec_ZZ_pX tb_temp;
        tb_temp.SetLength(2);
        if (d == 1) {
            ...
            for (long i = 0; i <= deg(tb_temp[0]); i++) {
                tb_temp[0].rep[i] *= conv<ZZ_p>(F[count]);
                tb_temp[1].rep[i] *= conv<ZZ_p>(F[count]);
            }
            tb[0] = tb[0] + tb_temp[0];
            tb[1] = tb[1] + tb_temp[1];
            count++;
        ...
        for (int i = beg_ind; i < num_data; i++) {
            ...
            f(tb, b, d, num_data, loop, i, ind_var,
              pkePara, modulus, ek, C_X, F, PRF, count);
        }
    }
}

void Compute(ZZ_pX &tby, ep_t g1T1, ep2_t g2T2, int b, CZ_Para pvhssPara, PKE_Para pkePara,
                ZZ_pXModulus modulus, vec_ZZ_pX ek, vec_ZZ_pX C_alpha,
                Vec<vec_ZZ_pX> C_X, vec_ZZ F, Vec<vec_ZZ_pX> PRF) {
    int count=0;
    int loop = 0;
    int beg_ind = 0;
    ...
    for (int i = 1; i < pkePara.d + 1; i++) {
        f(tb_temp, b, i, pkePara.num_data, loop, beg_ind, ind_var, pkePara, modulus, ek, C_X, F, PRF, count);
        prfkey = 1;
        tb[0] = tb[0] + tb_temp[0];
        tb[1] = tb[1] + tb_temp[1];
        tb_temp[0] = 0;
...
void Verify(ZZ_pX &y1, ZZ_pX &y2, ep_t g1T1, ep2_t g2T2, CZ_Para pvhssPara, PKE_Para pkePara, int &flag) {
    fp12_t gtT1;
    fp12_t gtT2;
    fp12_t right_side;
    fp12_t left_side;
